import 'package:sayaaratukum/ui/l10n/lang.dart';

class ArTranslations {
  static Map<String, String> get map => {
        L10n.viewAll: 'عرض الكل',
        L10n.brands: 'ماركات مشهورة',


        L10n.home: 'الرئيسيه',
        L10n.recommended: 'سيارات ينصح بها',
        L10n.spareParts: 'قطع غيار',
        L10n.stores: 'المعارض',
        L10n.favorite: 'المفضلات',


        L10n.myProfile: 'حسابي',
        L10n.setting: 'الاعدادات',
        L10n.privacy: 'الخصوصيه',
        L10n.socialMedia: 'التواصل الاجتماعي',
        L10n.logout: 'تسجيل خروج',


        L10n.welcomeTitle: 'لنبدأ تجربة جديدة',
        L10n.welcomeDoc: 'اختر سيارتك المفضلة، اشتريها واستمتع بتجربة جديدة!',
        L10n.letStart : 'لنبدأ',

        L10n.skip : 'تخطى',
        L10n.login : 'تسجيل الدخول',
        L10n.register : 'إنشاء حساب',
        L10n.email : 'البريد الإلكتروني',
        L10n.phoneNumber : 'رقم الهاتف',
        L10n.firstName : 'الاسم الأول',
        L10n.lastName : 'الاسم الأخير',


        L10n.alreadyHaveAccount : 'هل لديك حساب؟',
        L10n.createAccount : 'إنشاء حساب',
        L10n.forgotPassword : 'نسيت كلمة المرور؟',
        L10n.emailOrNumberPhone : 'البريد الإلكتروني أو رقم الهاتف',
        L10n.isRequired: 'مطلوب',
        L10n.password : 'كلمه السر',
        L10n.passwordConfirmation : 'تاكيد كلمه السر',
        L10n.emailIsInvalid: 'البريد الإلكتروني غير صالح',
        L10n.passwordShort: 'يجب أن تكون كلمة المرور القصيرة أكثر من 8 أحرف',
        L10n.passwordLong: 'يجب أن تكون كلمة المرور الطويلة أقل من 50 حرفًا',
        L10n.passwordNotMatch: 'كلمة المرور غير متطابقة',

        L10n.searchCar: 'ابحث عن سيارتك',
        L10n.searchTool: 'ابحث عن قطع غيار',

        L10n.showrooms: 'المعارض',
        L10n.adsDeals: 'عروض خاصة بالنسبة لك',
        L10n.specifications: 'المواصفات',
        L10n.note: 'ملاحظة',
        L10n.chatToBuy: 'الدردشة للشراء',

        L10n.make: 'الماركة',
        L10n.model: 'الموديل',
        L10n.status: 'الحالة',
        L10n.serialNumber: 'الرقم التسلسلي',
        L10n.color: 'اللون',
        L10n.madeTo: 'نوع الوارد',

        L10n.showMore: 'عرض المزيد',
        L10n.showLess: 'عرض أقل',
        L10n.addCar: 'إضافة سيارة',
        L10n.addTool: 'إضافة قطعة غيار',

        L10n.back: 'رجوع',
        L10n.next: 'التالي',
        L10n.publish: 'نشر',

        L10n.typeCar: 'نوع السيارة',
        L10n.numberRegisterCar: 'رقم تسجيل السيارة',
        L10n.propulsionSystem: 'نظام الدفع',
        L10n.year: 'السنة',
        L10n.fromYear: 'من السنة',
        L10n.toYear: 'إلى السنة',

        L10n.yearBrand: 'السنة مطلوبة!',

        L10n.recolor: 'تغيير اللون',
        L10n.km: 'كم',
        L10n.no: 'لا',
        L10n.yes: 'نعم',
        L10n.automatic: 'أوتوماتيك',
        L10n.manual: 'عادي',
        L10n.shakeCheck: 'فحص الهزات',
        L10n.gearBox: 'نوع الكير',
        L10n.isDamage: 'ضرر السيارة',


        L10n.governorate: 'المحافظة',
        L10n.region: 'المنطقة',
        L10n.nearPoint: 'اقرب نقطة',
        L10n.engineCapacity: 'سعة المحرك',
        L10n.enginePower: 'نوع الوقود',
        L10n.drivingMiles: 'عدد الأميال المقطوعة',

        L10n.informationCar: 'معلومات السيارة',
        L10n.locationCar: 'موقع السيارة',
        L10n.imagesCar: 'الصور والسعر',
        L10n.price: 'السعر',
        L10n.minPrice: 'أدنى سعر',
        L10n.maxPrice: 'أعلى سعر',
        L10n.addImagesCar: 'إضافة عشر صور للسيارة',
        L10n.addImageTool: 'إضافة صورة لقطعه الغيار',
        L10n.available: 'متاح',

        L10n.failedCreateAccount: 'فشل إنشاء الحساب',

        L10n.statusTool: 'حالة قطعة الغيار',
        L10n.name: 'الاسم',
        L10n.category: 'الفئة',
        L10n.imagesTool: 'صورة لقطعه غيار',
        L10n.description: 'الوصف',
        L10n.newTool: 'جديد',
        L10n.usedTool: 'مستعمل',
        L10n.failedPublishTool: 'فشل نشر الأداة',
        L10n.success: 'نجاح',
        L10n.successAddTool: 'تمت إضافة الأداة بنجاح',

        L10n.loginToAddFavorites: 'تسجيل الدخول لإضافة العناصر كمفضلة.',
        L10n.loginToShowNotification : 'سجل الدخول لمشاهده الاشعارات.',
        L10n.loginToBuy : 'سجل الدخول لبيع سيارتك/قطع غيارك',

        L10n.empty: 'فارغ',
        L10n.emptyFavorite: 'ابدأ بإضافة العناصر كمفضلة لرؤيتها هنا',


        L10n.cancel: 'إلغاء',
        L10n.areYouSure: 'هل أنت متأكد؟',
        L10n.messageDeleteAccount: 'هل ترغب حقًا في حذف حسابك؟ لن تتمكن من التراجع عن هذا الإجراء.',
        L10n.messageLogout: 'هل أنت متأكد من رغبتك في تسجيل الخروج؟',
        L10n.messageDeleteCar: 'هل أنت متأكد من رغبتك في بحذف السيارة؟',

        L10n.notAuth: 'يرجى تسجيل الدخول / التسجيل',

        L10n.successAddFavorite: 'تمت إضافة المفضلة بنجاح',
        L10n.successRemoveFavorite: 'تمت إزالة المفضلة بنجاح',
        L10n.failedAddFavorite: 'فشل إضافة المفضلة',

        L10n.check: 'تحقق',
        L10n.option: 'خيار',

        L10n.subscribe: 'اشتراك',
        L10n.subscribed: 'مشترك',
        L10n.mySubscribed: ' اشتراكاتي',
        L10n.myCars: ' سياراتي',
        L10n.notAddCarYet : 'لم تتم اضافه سيارة بعد ، اضف سيارة الان لمشاهدتها هنا',

        L10n.myStore: 'متجري',
        L10n.cars: 'السيارات',
        L10n.car : 'السيارة',
        L10n.updateCar : 'تحديث معلومات السيارة',
        L10n.deleteCar : 'حذف السيارة',
        L10n.tool : 'قطعة الغيار',
        L10n.tools: 'قطع الغيار',
        L10n.now: 'الآن',


        L10n.emptySubscribed: 'لم يتم الاشتراك في أي متجر بعد',
        L10n.noteEmptySubscribed: 'ابدأ الاشتراك في المتاجر وستظهر جميع المتاجر المشترك فيها هنا',


        L10n.filterBy: 'تصفية حسب',
        L10n.cleanFilter: 'مسح الكل',
        L10n.showResult: 'عرض النتائج',
        L10n.noResult: 'لا توجد نتائج',

        L10n.whatsappNotInstall: 'تطبيق WhatsApp غير مثبت على الجهاز. قم بتثبيت التطبيق وحاول مرة أخرى.',
        L10n.numberPhoneNotAdd: 'لم يتم إضافة رقم الهاتف.',
        L10n.whatsappNumberPhoneNotAdd: 'لم يتم إضافة رقم الهاتف لـ WhatsApp',

        L10n.storeExpires: 'انتهت صلاحية المتجر',
        L10n.notFound: 'غير موجود',
        L10n.profile: 'الملف الشخصي',
        L10n.updateProfile : 'تعديل الملف الشخصي',
        L10n.updatePassword : 'تغير كلمه السر',
        L10n.successfullyUpdateProfile : 'تم التعديل بنجاح',
        L10n.updateEmail : 'تغير البريد الاكتروني',
        L10n.successfullyUpdateEmail : 'تم تحديث البريد الاكتروني بنجاح',
        L10n.updatePhoneNumber : 'تعديل رقم الهاتف',
        L10n.successfullyUpdatePhoneNumber : 'تم التعديل بنجاح',
        L10n.save : 'حفظ',
        L10n.buyYourCar : 'بيع سيارتك',
        L10n.buyTool : 'بيع قطع غيار',

        L10n.faceBook : 'فيس بوك',
        L10n.twitter : 'تويتو',
        L10n.instagram : 'انستا',

        L10n.notifications : 'الاشعارات',
        L10n.language : 'اللغة',

        L10n.verifyEmail :  'Please verify your email',

        L10n.address :  'العنوان',
        L10n.bio: 'السيرة الذاتية',
        L10n.validateEmail :  'تاكيد البريد الالكتروني',
        L10n.confirmEmail :  'من فضلك اكد ايميلك الالكتروني',
        L10n.ok :  'تمام',
        L10n.cc :  'سي سي',
        L10n.or :  'او',
        L10n.notification :  'الاشعارات',
        L10n.thisIs :  'هذا',
        L10n.ist :  'تم',
        L10n.allNotificationIsRead :  'جعل الكل مقروءة',
        L10n.vinCheck: 'كشف حادث السيارة',
        L10n.vinNumber: 'رقم الشاصي',
        L10n.creditCard: 'الرقم التسلسلي للرصيد',
        L10n.creditType: 'نوع شركة الاتصال',
        L10n.emptyNotify:
            'لاتوجد اشعارات حاليا ',
        L10n.emptyNotifyDes:
            'اشترك بالمعارض لرسال اشعار عند اضافه عنصر جديد \n  او عند تحديث سيارة او قطعة غيار سيتم ارسال اشعار ',

        L10n.vinMessage : " وسيتم ارسال التقرير فور تجهيزه على واتس اب",
        L10n.internetIssue : "لايوجد اتصال بالانترنيت",


        L10n.carDeleted : 'تم حذف السيارة بنجاح',





        "Asiacell": "اسياسيل",
        "Zain": "زين",
        "Korek": "كورك",

        //income to
        "USA": 'امريكي',
        "Europe": 'اوربي',
        "Gulf": 'خليجي',
        "CA": 'كندي',

        //gov
        "Baghdad": "بغداد",
        "Basra": "البصرة",
        "Maysan": "ميسان",
        "Dhi Qar": "ذي قار",
        "Qadisiyah": "القادسية",
        "Wasit": "واسط",
        "Babylon": "بابل",
        "Najaf": "النجف",
        "Karbala": "كربلاء",
        "Al-Anbar": "الأنبار",
        "Salahuddin": "صلاح الدين",
        "Diyala": "ديالى",
        "Kirkuk": "كركوك",
        "Sulaymaniyah": "السليمانية",
        "Erbil": "أربيل",
        "Dohuk": "دهوك",
        "Halabja": "حلبجة",

       //colors
        "Black": "أسود",
        "White": "أبيض",
        "Silver": "فضي",
        "Gray": "رمادي",
        "Blue": "أزرق",
        "Red": "أحمر",
        "Green": "أخضر",
        "Yellow": "أصفر",
        "Orange": "برتقالي",
        "Brown": "بني",
        "Gold": "ذهبي",
        "Beige": "بيج",
        "Burgundy": "عنابي",
        "Purple": "أرجواني",
        "Pink": "وردي",
        "Champagne": "شامبانيا",
        "Bronze": "برونزي",
        "Charcoal": "فحمي",
        "Gunmetal": "معدني",
        "Navy Blue": "أزرق داكن",
        "Sky Blue": "أزرق سماوي",
        "Teal": "أزرق ملون",
        "Olive Green": "أخضر زيتوني",
        "Lime Green": "أخضر ليموني",
        "Amber": "عنبري",
        "Maroon": "كستنائي",
        "Ruby Red": "أحمر عنابي",
        "Crimson": "قرمزي",
        "Magenta": "قرنفلي",
        "Pearl White": "أبيض لؤلؤي"
      };
}
